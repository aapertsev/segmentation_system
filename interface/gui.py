...
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QMessageBox, QPlainTextEdit, QSlider, QListWidget,
    QListWidgetItem, QCheckBox, QLabel, QDialog, QTableWidgetItem,
    QTableWidget, QHeaderView, QVBoxLayout
)

from utils import convert_jpg_to_nii
...
class MainWindow(QMainWindow):
    ...
    Главное окно:
      - При "Запуск предсказания" пользователь указывает директорию с JPG.
        → Создаём подпапку "nii" (конвертация JPG -> NIfTI).
        → Создаём подпапку "results".
        → Запускаем nnUNetv2_predict -i <nii_dir> -o <results_dir>.
      - После завершения инференса автоматически открываем окно визуализации.
      - Кнопка "Визуализация" вручную делает то же самое (спрашивает путь и ищет results).
    """
    def __init__(self):
        super().__init__()
...
class VisualizationWindow(QWidget):
    """
    Окно визуализации (логика отображения изображений и масок, кнопок для карты плотности, гистограммы и т.д.).
    """
    ...
    def find_pairs(self):
        """
        Сопоставляем файлы patient_XXX_0000.nii.gz (в папке images_dir/nii) c patient_XXX.nii.gz (в папке masks_dir).
        """
    ...
    def build_density_map(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Карта плотности")
        label_map = QLabel()
    ...
    def save_report_to_pdf(self):
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            plt.text(0.5, 0.5, f"Отчет по {os.path.basename(self.images_dir)}",
            ...
            for pid in sorted(self.pairs.keys()):
                ...
                # Страница с таблицей плотностей
